...
=== Gradebook feature
==== Implementation
The gradebook feature allows users to add components, weightage and grades to a tracked module.
The user can use `grade` commands to add, edit or delete grades.

==== Design Considerations

===== Aspect: How to add components and grades.
* **Alternative 1 (current choice):** Add component first, then grade to the respective tracked module.
** Pros: User can key in component(s) and their weightage before knowing their actual grades, and system can check easily if components add up to 100%.
** Cons: We must ensure that the implementation of `addComponent` and `addGrade` are correct.

* **Alternative 2**: Add components and grades together.
** Pros: Only 1 method is required to perform this function.
** Cons:
...
[discrete]
=== Use case: Add a grade for a tracked module

*MSS*

1.  User requests to view the specific tracked module
2.  ModuleBook displays the module details
3.  User requests to add a grade
4.  ModuleBook adds the grade
+
Use case ends.
